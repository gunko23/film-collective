...
export default function RecommendationEngineVisual() {
      ...
      {activeTab === "pipeline" && (
        <div>
          <p style={{ fontSize: 12, color: C.textDim, textAlign: "center", marginBottom: 14, marginTop: 0 }}>
            Tap any phase to expand · Highlighted items are internal-DB-powered signals
          </p>
          {phases.map((phase, i) => (
            <div key={phase.key}>
              <PhaseCard
      ...
      {activeTab === "scoring" && (
        <div>
          <p style={{ fontSize: 12, color: C.textDim, textAlign: "center", marginBottom: 16, marginTop: 0 }}>
            Every signal in <span style={{ fontFamily: "'Space Mono', monospace", color: C.textSoft }}>calculateGroupFitScore()</span> — applied top to bottom
          </p>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginTop: 12 }}>
            ...
            <div style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.7 }}>
              Base (55) → Genre preference (+3 base, then weighted match) → Disliked genre penalty →
              Seen penalty → Member endorsement → Shuffle / history penalties → <span style={{ color: C.purple }}>Mood bonus + threshold penalty</span> →
              Quality bonus → Acclaimed bonus → Audience penalty → Availability + vote confidence →
              Director affinity → Actor affinity → Era preference → Collaborative filtering →
              Collective influence → Internal DB signal → <span style={{ color: C.purple }}>Mood dampening (multiplicative)</span> → Well-rounded bonus → Clamp 0–100
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
            ...
            <div style={{
              ...
              <span style={{ fontWeight: 600, color: C.text }}>Pool assembly:</span> Internal DB seeds the array first → TMDB results appended →
              Deduplication by TMDB ID (TMDB metadata wins on overlap) → Quality gate removes thin entries →
              Typical viable pool after dedup + filtering: 150–300 unique movies.
            </div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.6, marginBottom: 14 }}>
              The mood system operates at three levels — it shapes which movies enter the pool, how they're scored, and which survive the final gate.
            </div>
            {[
              {
                level: "Sourcing",
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.5 }}>
              ...
              data is unavailable. Composite score maps to bonus tiers: 85+ → +15, 75+ → +12, 65+ → +8, 50+ → +5.
            </div>
          </div>
