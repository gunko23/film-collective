...
// ── Spinning Film Reel ──
function FilmReel({ size = 120, speed = 3 }) {
  const spokeCount = 5;
  const outerR = size / 2;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}
      ...
      {/* Outer rim */}
      <circle cx={outerR} cy={outerR} r={outerR - 2} fill="none"
        stroke="#D4753E" strokeWidth={rimWidth} opacity="0.2" />
      <circle cx={outerR} cy={outerR} r={outerR - 2} fill="none"
        stroke="#D4753E" strokeWidth="2" opacity="0.6" />
      ...
      {Array.from({ length: 16 }).map((_, i) => {
        ...
        return (
          <circle key={`hole-${i}`} cx={cx} cy={cy} r={rimWidth * 0.22}
            fill="#0D0B09" stroke="#D4753E" strokeWidth="0.5" opacity="0.4" />
        );
      })}

      {/* Inner area */}
      <circle cx={outerR} cy={outerR} r={innerR + 4} fill="none"
        stroke="#D4753E" strokeWidth="1.5" opacity="0.3" />
      ...
      {Array.from({ length: spokeCount }).map((_, i) => {
        ...
        return (
          <line key={`spoke-${i}`} x1={x1} y1={y1} x2={x2} y2={y2}
            stroke="#D4753E" strokeWidth="3" strokeLinecap="round" opacity="0.5" />
        );
      })}

      {/* Center hub */}
      <circle cx={outerR} cy={outerR} r={hubR} fill="#0D0B09"
        stroke="#D4753E" strokeWidth="2" opacity="0.7" />
...
function FilmStrip({ delay = 0, top = "20%", direction = 1 }) {
  const frameCount = 5;
  const frameW = 32;
  const frameH = 24;
  const gap = 4;
      ...
      <svg width={stripW} height={stripH} viewBox={`0 0 ${stripW} ${stripH}`}>
        ...
        {Array.from({ length: frameCount * 2 + 1 }).map((_, i) => (
          <rect key={`ht-${i}`}
            ...
            y="2" width={holeSize} height={holeSize} rx="1" fill="#0D0B09" />
        ))}
        ...
        {Array.from({ length: frameCount * 2 + 1 }).map((_, i) => (
          <rect key={`hb-${i}`}
            x={gap + i * ((stripW - gap * 2) / (frameCount * 2))}
            y={stripH - holeSize - 2} width={holeSize} height={holeSize} rx="1" fill="#0D0B09" />
        ))}

        {/* Frames */}
        {Array.from({ length: frameCount }).map((_, i) => (
          <rect key={`frame-${i}`}
            x={gap + i * (frameW + gap)} y={8}
            width={frameW} height={frameH}
            rx="1" fill="#0D0B09" opacity="0.7" />
        ))}
      </svg>
    </div>
  );
...
function ProjectorBeam() {
  return (
    <div style={{
      ...
      width: "150%",
      height: "55%",
      background: "conic-gradient(from 170deg at 50% 0%, transparent 35%, rgba(212,117,62,0.03) 45%, rgba(212,117,62,0.06) 50%, rgba(212,117,62,0.03) 55%, transparent 65%)",
      pointerEvents: "none",
...
export default function TonightsPickLoading() {
    ...
    <div style={{
      ...
      <style>{`
        ...
        @keyframes countdownPulse {
          ...
          40% { transform: scale(1.1); opacity: 0.06; }
